...
def load_data_nc(dataset, use_feats, data_path, split_seed):
    if dataset in ['cora', 'pubmed']:
        ...
        if dataset == 'disease_nc':
            ...
            adj, features, labels = load_data_ppi(dataset, data_path, return_label=True)
            val_prop, test_prop = 0.15, 0.15
        else:
            raise FileNotFoundError('Dataset {} is not supported.'.format(dataset))
...
def load_data_ppi(dataset_str, data_path, return_label=False):
    adj = np.load(os.path.join(data_path, 'adj.npy'))
    features = np.load(os.path.join(data_path, 'feat.npy'))
